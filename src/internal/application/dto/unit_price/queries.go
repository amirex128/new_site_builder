...
type CalculateUnitPriceUnitPriceQuery struct {
	UnitPriceName  *UnitPriceNameEnum `json:"unitPriceName" validate:"required"`
	UnitPriceCount *int               `json:"unitPriceCount" validate:"required,gt=0"`
	UnitPriceDay   *int               `json:"unitPriceDay,omitempty" validate:"required_if=UnitPriceName StorageMbCredits"`
}

// CalculateUnitPriceQuery represents a query to calculate unit prices
type CalculateUnitPriceQuery struct {
	UnitPrices []CalculateUnitPriceUnitPriceQuery `json:"unitPrices" validate:"required,min=1"`
}

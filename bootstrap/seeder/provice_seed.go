...
func (s *seeder) provinceSeeder() error {
	var cities []domain.Province
	err := json.Unmarshal([]byte(cityJson), &cities)
	if err != nil {
		return err
	}
	err = s.container.GetProvinceRepo().CreateMany(cities)
	if err != nil {
		return err
	}
